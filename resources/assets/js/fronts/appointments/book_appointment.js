        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    $('#appointmentServiceId').selectpicker('refresh')
                }
            },
        })
    }

    if (!($('#appointmentServiceId').val() == '') && $('#appointmentServiceId').length) {
        $.ajax({
            url: route('get-charge'),
            type: 'GET',
            data: {
                'chargeId': $('#appointmentServiceId').val(),
            },
            success: function (result) {
                if (result.success) {
...
let serviceIdExist = $('#appointmentServiceId').val()
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.success) {
                ...
                $('#appointmentServiceId').append($('<option value=""></option>').text('Select Service'))
                $.each(result.data, function (i, v) {
                    $('#appointmentServiceId').append($('<option></option>').attr('value', v.id).attr('selected', v.id == serviceIdExist).text(v.name))
                })
                if (serviceIdExist && $('#appointmentServiceId').val()) {
                    $('#payableAmountText').removeClass('d-none')
                }
                $('#appointmentServiceId').selectpicker('refresh')
            }
...
listenSubmit('#frontAppointmentBook', function (e) {
    ...
    let services = $('#appointmentServiceId').val().trim()
