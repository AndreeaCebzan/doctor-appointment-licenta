<a href="{{ route('doctors.appointments.calendar') }}"
   class="btn btn-icon btn-light me-2 ms-auto"><i
            class="fas fa-calendar-alt fs-2"></i></a>
<div class="me-4" wire:ignore>
            ...
            <div class="mb-10">
                ...
                <div>
                    <input type="text" class="form-control form-control-solid flatpickr-input"
                           placeholder="Pick date rage" id="doctorPanelAppointmentDate"/>
                </div>
            </div>
            <div class="mb-10">
                <label class="form-label fw-bold">{{__('messages.appointment.payment')}}</label>
                <div>
                    {{ Form::select('payment_type', \App\Models\Appointment::PAYMENT_TYPE_ALL, \App\Models\Appointment::ALL_PAYMENT,['class' => 'form-control form-control-solid form-select', 'data-control'=>"select2", 'id' => 'doctorPanelPaymentType']) }}
                </div>
            </div>
            <div class="mb-10">
                <label class="form-label fw-bold">{{__('messages.doctor.status')}}</label>
                <div>
                    {{ Form::select('status', $appointmentStatus, \App\Models\Appointment::BOOKED,['class' => 'form-control form-control-solid form-select', 'data-control'=>"select2", 'id' => 'doctorPanelAppointmentStatus']) }}
                </div>
            </div>
            <div class="d-flex justify-content-end">
                <button type="reset" id="doctorPanelApptmentResetFilter"
