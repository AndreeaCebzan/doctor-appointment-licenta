            ...
            <form class="row mb-0" id="frontAppointmentBook" action="{{ route('front.appointment.book') }}" method="post">
                ...
                <div class="col-md-6 form-group text-start">
                    {{ Form::label('Doctor',__('messages.doctor.doctor').':') }}<span
                            class="required text-danger">*</span>
                    {{ Form::select('doctor_id', $appointmentDoctors, isset(session()->get('data')['doctor_id']) ? session()->get('data')['doctor_id'] : '',['class' => 'form-select', 'id' => 'appointmentDoctorId', 'data-control'=>"select2",'placeholder' => 'Select Doctor']) }}
                </div>
                <div class="col-md-6 form-group text-start">
                    {{ Form::label('Service',__('messages.appointment.service').':') }}<span
                            class="required text-danger">*</span>
                    {{ Form::select('service_id', isset(session()->get('data')['service']) ? session()->get('data')['service'] : [] , isset(session()->get('data')['service_id']) ? session()->get('data')['service_id'] : '',['class' => 'form-select', 'data-control'=>"select2", 'id'=> 'appointmentServiceId','placeholder' => 'Select Service']) }}
                </div>
                <div class="col-md-6 form-group text-start">
...
<div class="fc-timegrid-slot  not-dark form-control form-control-solid h-300px overflow-auto">
    ...
    <div class="text-center d-flex flex-wrap justify-content-center px-3 max-height-235px front-slot-data appointment-slot-data"
         id="slotData"></div>
