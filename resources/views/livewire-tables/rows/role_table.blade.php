...
<x-livewire-tables::bs5.table.cell>
    @forelse($row->permissions as $key => $permission)
        <a class="badge badge-light-{{ getBadgeColor($key) }} fs-7 m-1">{{$permission->display_name}}</a>
    @empty
...
<x-livewire-tables::bs5.table.cell>
    @if(!$row->is_default && getLogInUser()->hasRole('clinic_admin'))
        <div class="d-flex justify-content-center">
            <a title="{{__('messages.common.edit')}}" href="{{ route('roles.edit', $row->id) }}"
            ...
            <a title="{{__('messages.common.delete')}}" href="javascript:void(0)" data-id="{{ $row->id }}"
               class="btn role-delete-btn btn-icon btn-bg-light text-hover-danger btn-sm ms-1" data-turbolinks="false">
                        <span class="svg-icon svg-icon-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24"
                             version="1.1">
