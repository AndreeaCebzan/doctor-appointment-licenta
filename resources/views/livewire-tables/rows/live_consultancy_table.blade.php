<x-livewire-tables::bs5.table.cell>
    <a href="javascript:void(0)" class="consultation-show-data" data-id="{{ $row->id }}">
...
<x-livewire-tables::bs5.table.cell>
    ...
    <div class="badge badge-light">
        <div class="mb-2">
            {{ \Carbon\Carbon::parse($row->consultation_date)->format('H: i A') }}
        </div>
        <div>
            {{ \Carbon\Carbon::parse($row->consultation_date)->format('jS M, Y') }}
        </div>
    </div>
</x-livewire-tables::bs5.table.cell>
...
<x-livewire-tables::bs5.table.cell>
    ...
    @if( auth()->user()->hasAnyRole('doctor', 'admin') )
        <div class="w-150px d-flex align-items-center">
            <span class="slot-color-dot bg-{{ $colors[$row->status] }} rounded-circle me-2"></span>
            <select class="form-select-sm form-select-solid form-select consultation-change-status"
    ...
    @else
        @if( $row->status == 1 )
            <span class="badge badge-light-danger fw-bolder ms-2 fs-8 py-1 px-3">Cancelled</span>
        @elseif( $row->status == 0 )
            <span class="badge badge-light-warning fw-bolder ms-2 fs-8 py-1 px-3">Awaited</span>
        @else
            <span class="badge badge-light-success fw-bolder ms-2 fs-8 py-1 px-3">Finished</span>
        @endif
    @endif
</x-livewire-tables::bs5.table.cell>
...
<x-livewire-tables::bs5.table.cell>
    <div class="d-flex justify-content-center">
        @if( $row->status == 0 )
            <a title="{{ getLogInUser()->hasRole('patient') ? 'Join Meeting' : 'Start Meeting' }}"
               class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm start-btn" data-id="{{  $row->id }}">
        ...
        @if(getLogInUser()->hasRole('doctor') )
            @if( $row->status != 2 )
                <a href="javascript:void(0)" title="{{ __('messages.common.edit') }}"
                   class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm ms-2 live-consultation-edit-btn"
                   data-id="{{ $row->id }}">
            ...
            <a href="javascript:void(0)" title="{{ __('messages.common.delete') }}" data-id="{{ $row->id }}"
               class="live-consultation-delete-btn btn btn-icon btn-bg-light btn-active-color-danger btn-sm ms-2">
            <span class="svg-icon svg-icon-3">
