...
<x-livewire-tables::bs5.table.cell>
    <div class="text-center badge badge-light-danger mx-auto d-block"
         style="width: fit-content;">{{ $row->appointments_count }}</div>
</x-livewire-tables::bs5.table.cell>

<x-livewire-tables::bs5.table.cell>
    <div class="form-check form-switch form-check-custom form-check-solid justify-content-center">
        <input class="form-check-input h-20px w-30px patient-email-verified" data-id="{{ $row->user->id }}"
               type="checkbox" value=""
    ...
    <div class="d-flex justify-content-center">
        @if(empty($row->user->email_verified_at))
            <a href="#" data-id="{{ $row->user->id }}"
               class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm patient-email-verification"
        ...
        <a title="{{__('messages.common.edit')}}" href="{{ route('patients.edit', $row->id) }}"
        ...
        <a title="{{__('messages.common.delete')}}" href="javascript:void(0)" data-id="{{ $row->id }}"
           class="btn patient-delete-btn btn-icon btn-bg-light text-hover-danger btn-sm ms-2" data-turbolinks="false">
                        <span class="svg-icon svg-icon-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24"
                             version="1.1">
